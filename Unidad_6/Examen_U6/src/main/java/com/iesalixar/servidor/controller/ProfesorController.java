...
import com.iesalixar.servidor.dto.ProfesorAsignaturaDTO;
...
import com.iesalixar.servidor.service.AsignaturaServiceImpl;
import com.iesalixar.servidor.service.DepartamentoServiceImpl;
...
public class ProfesorController {
	...
	public String addProfesorGet(@RequestParam(required = false, name = "error") String error, Model model){
		...
		List<Departamento> departamentos = deptService.getAllDepartamentos();
	...
	public String addProfesorPost(@ModelAttribute ProfesorDTO profesor,Model model) {
		...
		profesorBD.setDepartamento(profesor.getDepartamento());
	...
	public String addAsigProGet(@RequestParam(required=false,name="error") String error, Model model) {
	...
	public String addAsigPro(@ModelAttribute ProfesorAsignaturaDTO asigProf,Model model) {
		
		Optional<Profesor> prof = profesorService.findProfesorById(asigProf.getId_profesor());
		Optional<Asignatura> asig = asignaturaService.findAsignaturaById(asigProf.getId_asignatura());
		
		if(prof != null && asig != null) {
			
			asig.get().setProfesor(prof.get());;
			asignaturaService.actualizarAsignatura(asig.get());
			
			return "redirect:/profesores/asignaturas?codigo="+prof.get().getId();
