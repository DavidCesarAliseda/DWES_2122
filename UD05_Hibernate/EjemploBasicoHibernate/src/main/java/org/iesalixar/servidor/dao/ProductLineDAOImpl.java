...
public class ProductLineDAOImpl implements ProductLineDAO {
	...
	public List<ProductLine> getAllProductLine() {
		...
		List<ProductLine> categorias = 
				session.createNativeQuery("select * from productlines", 
						ProductLine.class).list();

		 //Versión HQL
		 //List<ProductLine> categorias = session.createQuery("FROM productLine").list();
